...
def plot_ell(SEMA, ECC, INC, PHA, IND=[1],label=None, proj=None):
    """
    ...
    "
    Very confusing and uncessarily complicated code.
    This function does not work without  the variables size_SEMA. I can guess size_SEMA = np.shape(SEMA). 
  
    I just saw that n is coming from the function call as below 
    t = numpy.random.random([2, 4, 5, 2])
    n = sub2ind(numpy.shape(t), [1, 2, 4, 1])

    The problem/question is that is n in the workspace?

    Generally,a very bad idea for code redeadability.

    Why all this trouble to calculate the indices? And write titles?
    n = sub2ind(numpy.shape(t), [1, 2, 4, 1])
    "
       -Ze Jan 2022
    ...
    if len_IND>0:
        ...
        for k in range(len_IND):
            if k == 0:
                titletxt = f'Ellipse ({IND[k]})'
                n = sub2ind(size_SEMA, [IND[k]])
                # print('n for k = 0, ',n)
            else:
                titletxt = f'Ellipse ({IND[0:k]})'
                n = sub2ind(size_SEMA, IND[0:k])
                # print(f'n for k ={k}, ',n)

        fig, ax = do_the_plot(SEMA.flatten()[n], ECC.flatten()[n], INC.flatten()[n], PHA.flatten()[n],label=None,proj=proj)
        titletxt = f'Ellipse {tuple(IND[0:k+1])},  (green: CW, red: CCW)'
        ax.set_title(titletxt)
        return fig, ax
    else:
         print('IND input contains zero element(s)!\nNo ellipse will be plotted.')

# rAu, rPhi_u, rAv, rPhi_v, rw = ep2ap(SEMA, ECC, INC, PHA,[])
...
#------------------------------------------------------------------------------
    ...
    #     # plt.gcf()
    #     # plt.clf()
    #     fig, ax = do_the_plot(SEMA.flatten()[n], ECC.flatten()[n], INC.flatten()[n], PHA.flatten()[n])
    #     titletxt = titletxt + ',  (green: CW, red: CCW component)'
    #     ax.set_title(titletxt)
    # elif len_IND:
    #     print('IND input contains zero element(s)!\nNo ellipse will be plotted.')
    # return fig, ax, cmd
...
def do_the_plot(SEMA, ECC, INC, PHA,label=None,proj=None):
    """
    Ellipse plot subfunction.

    Converted to Python by Pierre Cazenave, October 2012.

    improved figure returned by ze Jan 2022

    """

    fig, ax = plt.subplots(1,1,figsize=(5,5),dpi = 80, subplot_kw=dict(projection=proj) )
    i = 1j
    ...
    ax.plot(np.real(w), np.imag(w))
    ax.axis('equal')
#     ax.hold('on')     # COMMENTED OUT SINCE NO SUCH FEATURE IN MATPLOTLIB - ZE 2021-06
    ax.plot([0, np.real(wmax)], [0, np.imag(wmax)], 'm')
    ax.plot([0, np.real(wmin)], [0, np.imag(wmin)], 'm')
    ax.set_xlabel('u')
    ax.set_ylabel('v')
    ax.plot(np.real(a), np.imag(a), 'r')
    ax.plot(np.real(b), np.imag(b), 'g')
    ax.plot([0, np.real(a[0])], [0, np.imag(a[0])], 'r.')
    ax.plot([0, np.real(b[0])], [0, np.imag(b[0])], 'g.')
    ax.plot([0, np.real(w[0])], [0, np.imag(w[0])], 'b.',label=label)
    ax.plot(np.real(a[0]), np.imag(a[0]), 'r.')
    ax.plot(np.real(b[0]), np.imag(b[0]), 'g.')
    ax.plot(np.real(w[0]), np.imag(w[0]), 'b.')
    ax.plot(np.real([a[0], a[0]+b[0]]), np.imag([a[0], a[0]+b[0]]), linestyle='--', color='g')
    ax.plot(np.real([b[0], a[0]+b[0]]), np.imag([b[0], a[0]+b[0]]), linestyle='--', color='r')

    for n in range(len(ot)):
#         ax.hold('on')
        ax.plot(np.real(a[n]), np.imag(a[n]), 'r.')
        ax.plot(np.real(b[n]), np.imag(b[n]), 'g.')
        ax.plot(np.real(w[n]), np.imag(w[n]), 'b.')
